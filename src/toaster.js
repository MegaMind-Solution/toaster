 ...
 * ------------------------------------------------------------------
 * A zero-dependency, API-driven library for creating rich, interactive tooltips.
 * - API Driven: `init`, `destroy`, `updateConfig`
 * - Shadow DOM & MutationObserver Support for Web Apps & Extensions
 * - New Features: Lists, Icons, Images, Links, Dividers, Pos, Width
 * ------------------------------------------------------------------
 */
(function(window) {
    'use strict';
    ...
    const UniversalToaster = {
        ...
        /**
         * Initializes the library and starts listening for tooltips.
        ...
        init(userConfig = {}) {
            ...
            this._scanNode(document.body); // Initial scan
            this.isInitialized = true;
        },

        /**
         * Disables the library and cleans up all resources.
         */
        destroy() {
            if (!this.isInitialized) return;
         ...
         * @param {object} userConfig - Configuration object to merge.
         */
        ...
        _attachListeners() {
            // Bind `this` context for easy removal
        ...
        /** Scans a DOM node for titles and attaches listeners to shadow roots. */
        _scanNode(node) {
            if (node.nodeType !== Node.ELEMENT_NODE) return;
            // Scan for titles in the node itself
            if (node.hasAttribute('title')) {
                // Pre-swap to prevent native tooltip flash
                this._swapTitle(node);
            }
            // Scan for titles in children
        ...
        _onMouseOver(e) {
            ...
            this._showTimeout = setTimeout(() => {
                if (this._activeElement !== target) return; // Mouse moved away before delay finished
                ...
                // Store custom position for use in mousemove
        ...
        _applyTheme() { /* ... same as before, truncated for brevity ... */ if(this.config.backgroundColor){this._tooltipElement.style.backgroundColor=this.config.backgroundColor;this._tooltipElement.style.color=this.config.textColor;}else{let s=window.getComputedStyle(document.body),c=s.backgroundColor;if(c==='rgba(0, 0, 0, 0)'||c==='transparent')c='rgb(255, 255, 255)';const r=c.match(/\d+/g);let i=true;if(r){const b=Math.round(((parseInt(r[0])*299)+(parseInt(r[1])*587)+(parseInt(r[2])*114))/1000);if(b<125)i=false}if(i){this._tooltipElement.style.backgroundColor='#222';this._tooltipElement.style.color='#fff'}else{this._tooltipElement.style.backgroundColor='#fff';this._tooltipElement.style.color='#000'}}},
        ...
        _parseSyntax(rawText) {
            ...
            // --- Block-level & Behavioral commands ---
